 ...
 *
 * <p>This implementation performs all computations using instances of double-double numbers.
 * Results are returned as a new double-double instance.
...
//    /** Count of DD2 objects. */
//    private static long dd;
//    /** Count of MDD2 objects. */
//    private static long mdd;
     ...
     */
    private static final class MDD3 {
        /** The high part of the double-double number. */
        private double hi;
        /** The low part of the double-double number. */
        private double lo;
...
//            mdd++;
     ...
     * @param lo Low part.
     */
    private DD3(double hi, double lo) {
        this.hi = hi;
        this.lo = lo;
//        dd++;
    }
...
//     * @return the counters
//     */
//    static long[] getCounters() {
//        return new long[] {dd, mdd};
//    }
     ...
     * @return the double-double
     */
    private static DD3 create(double a, double b) {
        return twoSum(a, b);
    }
     ...
     */
    DD3 negate() {
        return new DD3(-hi, -lo);
    }

    /**
     * Subtract the value {@code x} from the low part.
     *
     * @param x Value.
     * @return the result
     */
    private DD3 subtractLow(double x) {
        return new DD3(hi, lo - x);
    }
     ...
     */
    static DD3 fastTwoSum(double a, double b) {
        // (x, xx) = a + b
        // bVirtual = x - a
        // xx = b - bVirtual
        final double x = a + b;
        final double lo = b - (x - a);
        return new DD3(x, lo);
    }
     ...
     * @see #fastTwoSum(double, double, DD3)
     */
     ...
     * Shewchuk (1997) Theorum 6</a>
     */
    static DD3 fastTwoDiff(double a, double b) {
        // (x, xx) = a - b
        // bVirtual = a - x
        // xx = bVirtual - b
        final double x = a - b;
        final double lo = (a - x) - b;
        return new DD3(x, lo);
    }
     ...
     */
    static DD3 twoSum(double a, double b) {
        // (x, xx) = a + b
        // bVirtual = x - a
        // aVirtual = x - bVirtual
        ...
        final double lo = (a - (x - bVirtual)) + (b - bVirtual);
        return new DD3(x, lo);
    }
     ...
     */
    private static DD3 twoDiff(double a, double b) {
        // (x, xx) = a - b
        // bVirtual = a - x
        // aVirtual = x + bVirtual
        // bRoundoff = b - bVirtual
    ...
    private static DD3 twoProd(double x, double y) {
        ...
        if (a + b + Math.abs(xy) >= SAFE_UPPER) {
            ...
            if (a > b) {
                lo = productLowUnscaled(x * DOWN_SCALE, y, xy * DOWN_SCALE) * UP_SCALE;
            } else {
                lo = productLowUnscaled(x, y * DOWN_SCALE, xy * DOWN_SCALE) * UP_SCALE;
            }
        } else {
            // No scaling required. This is the expected branch for a finite product.
            lo = productLowUnscaled(x, y, xy);
        }
        return new DD3(xy, lo);
    }
    ...
    /**
     * Compute the sum of {@code this} and {@code y}.
     *
     * <p>This computes the same result as
     * {@link #fastAdd(DD3) add(DD.create(y))}.
     *
     * @param y y.
     * @return the sum
     * @see #fastAdd(DD3)
     */
    DD3 fastAdd(double y) {
        return fastAdd(hi, lo, y);
    }
     ...
     *
     * <p>This computes the same result as
     * {@link #fastAdd(double, double, double, double) add(x, xx, y, 0)}.
     *
     * @param x High part of x.
     * @param xx Low part of x.
     * @param y y.
     * @return the sum
     * @see #fastAdd(double, double, double, double)
     */
    private static DD3 fastAdd(double x, double xx, double y) {
        // (s0, s1) = x + y
        final double s0 = x + y;
     ...
     *
     * <p>This computes the same result as
     * {@link #add(double, double, double, double) add(x, xx, y, 0)}.
     *
     * @param x High part of x.
     * @param xx Low part of x.
     * @param y y.
     ...
     */
    private static DD3 add(double x, double xx, double y) {
        ...
        s = fastTwoSum(s1, s2);
        s2 = s.lo;
        s = fastTwoSum(s0, s.hi);
        // Here (s0, s1) = s
        // e = exact 159-bit result
        // |e - s0| <= ulp(s0)
        // |s1 + s2| <= ulp(e - s0)
        return fastTwoSum(s.hi, s2 + s.lo);
    }
     ...
     */
    DD3 fastAdd(DD3 y) {
        return fastAdd(hi, lo, y.hi, y.lo);
    }
     ...
     * @see #add(double, double, double, double)
     */
    private static DD3 fastAdd(double x, double xx, double y, double yy) {
        // Sum parts and save
        // (p, pp) = x + y
        DD3 s = twoSum(x, y);
        final double p = s.hi;
        ...
        s = fastTwoSum(p, pp + q);
        return fastTwoSum(s.hi, s.lo + qq);
    }
     ...
     * @see #fastAdd(double, double, double, double)
     */
    private static DD3 add(double x, double xx, double y, double yy) {
        ...
        s = fastTwoSum(s.lo, s2);
        final double s1 = s.hi;
        s = fastTwoSum(s.lo, s3);
        // s2 = s.hi
        s3 = s.lo;
        s = fastTwoSum(s1, s.hi);
        s2 = s.lo;
        s = fastTwoSum(s0, s.hi);
        // Here (s0, s1) = s
        ...
        return fastTwoSum(s.hi, s3 + s2 + s.lo);
    }

    /**
     * Compute the multiplication product of {@code this} and {@code y}.
     *
     * <p>This computes the same result as
     * {@link #multiply(DD3) multiply(DD3.create(y))}.
     *
     * @param y High part of y.
     * @return the product
     * @see #multiply(DD3)
     */
    DD3 multiply(double y) {
        return multiply(hi, lo, y);
    }
     ...
     */
    DD3 multiply(DD3 y) {
        return multiply(hi, lo, y.hi, y.lo);
    }
     ...
     * @return the product
     */
    private static DD3 multiply(double x, double xx, double y, double yy) {
        // Dekker mul2
        // (Alternative: Scale expansion (Schewchuk Fig 13))
        final DD3 p = twoProd(x, y);
        return fastTwoSum(p.hi, p.lo + (x * yy + xx * y));
    }
     ...
     *
     * <p>This computes the same result as
     * {@link #multiply(DD3) multiply(y)} without checks for overflow of intermediates.
     * An exception is if either argument is a signed zero; in this case the sign of the zero
     ...
     * @see #multiply(DD3)
     */
    DD3 uncheckedMultiply(DD3 y) {
        return uncheckedMultiply(hi, lo, y.hi, y.lo);
    }
     ...
     *
     * <p>This computes the same result as
     * {@link #multiply(double, double, double, double) multiply(x, xx, y, yy)}
     * without checks for overflow of intermediates. An exception is if either
     ...
     * @see #multiply(double, double, double, double)
     */
    private static DD3 uncheckedMultiply(double x, double xx, double y, double yy) {
        // Dekker mul2
        final double hi = x * y;
     ...
     * @return the quotient
     */
    private static DD3 divide(double x, double y) {
        // Long division
        // quotient q0 = x / y
        final double q0 = x / y;
        // remainder r = x - q0 * y
     ...
     * If {@code y = 0} the result is undefined.
     *
     * <p>This computes the same result as {@link #divide(double, double) divide(x, y)}
     ...
     * @return the quotient
     */
    static DD3 uncheckedDivide(double x, double y) {
        // Long division
        // quotient q0 = x / y
        final double q0 = x / y;
        // remainder r = x - q0 * y
    ...
    /**
     * Compute the division of {@code this} by {@code y}.
     * If {@code y = 0} the result is undefined.
     *
     * @param y y.
     * @return the quotient
     ...
     * @return the quotient
     */
    private static DD3 divide(double x, double xx, double y, double yy) {
        // Long division
        // quotient q0 = x / y
        final double q0 = x / y;
        // remainder r0 = x - q0 * y
        DD3 q = multiply(y, yy, q0);
        q = add(-q.hi, -q.lo, x, xx);
        final double r = q.hi;
        ...
        q = multiply(y, yy, q1);
        q = add(-q.hi, -q.lo, r, rr);
        // next quotient q2 = r1 / y
        final double q2 = q.hi / y;
        // Collect (q0, q1, q2)
        q = fastTwoSum(q0, q1);
        return twoSum(q.hi, q.lo + q2);
    }
     ...
     * @return the inverse
     */
    private static DD3 inverse(double y, double yy) {
     ...
     * @return the result
     * @see Math#scalb(double, int)
     * @see #frexp(double, double, int[])
     * @see <a href="https://www.cplusplus.com/reference/cmath/ldexp/">C math.h ldexp</a>
     */
    private static DD3 ldexp(double x, double xx, int exp) {
     ...
     * @see #frexp(int[])
     */
    DD3 fastPowScaled(int n, long[] exp) {
        return fastPowScaled(hi, lo, n, exp);
    }
     ...
     * @see #frexp(double, double, int[])
     */
    private static DD3 fastPowScaled(double x, double xx, int n, long[] exp) {
        // Edge cases.
        if (n == 0) {
            exp[0] = 1;
            return new DD3(0.5, 0);
        }
        // IEEE result for non-finite or zero
        if (!Double.isFinite(x) || x == 0) {
            exp[0] = 0;
            return new DD3(Math.pow(x, n), 0);
        }
        // Here the number is non-zero finite
        assert x == x + xx : NOT_NOMALIZED;
        final int[] ie = {0};
        DD3 f = new DD3(x, xx).frexp(ie);
        long b = ie[0];
        ...
        if (Math.abs(f.hi) == HALF && f.lo == 0) {
            ...
            final double hi = 0.5 * Math.pow(2 * f.hi, n);
            // Propagate sign change (x*f.hi) to the zero
            final double lo = Math.copySign(0.0, x * f.hi * xx);
            exp[0] = 1 + (b - 1) * n;
            return new DD3(hi, lo);
        }
        if (n < 0) {
            f = computeFastPowScaled(b, f.hi, f.lo, -n, exp);
        ...
        return computeFastPowScaled(b, f.hi, f.lo, n, exp);
    }
     ...
     * @return Fraction part.
     */
    private static DD3 computeFastPowScaled(long b, double x, double xx, int n, long[] exp) {
     ...
     * @see #frexp(int[])
     */
    DD3 powScaled(int n, long[] exp) {
        return powScaled(hi, lo, n, exp);
    }
     ...
     * @see #frexp(double, double, int[])
     */
    private static DD3 powScaled(double x, double xx, int n, long[] exp) {
        // Edge cases.
        if (n == 0) {
            exp[0] = 1;
            return new DD3(0.5, 0);
    ...
    private static DD3 computePowScaled(long b, double x, double xx, int n, long[] exp) {
        ...
        for (int i = 32 - shift; i != 0; i--, bits <<= 1) {
            ...
            if (bits < 0) {
                ...
                t = fastAdd(s2, r3b, u, v);
                s2 = t.hi + r2;
                r3b = twoSumLow(t.hi, r2, s2);
                // Sum (p21, q11, q20, r3a, r3b) -> s3             Order(eps^3)
     ...
     * @return s0
     */
    private static double norm3(double s0, double s1, double s2, double s3, MDD3 s12) {
        double q;
     ...
     * If {@code y = 0} the result is undefined.
     *
     * <p>This is special routine used in {@link #powScaled(double, double, int, DD3)}
     ...
     * @return the inverse
     */
    private static DD3 inverse3(double y, double yy, double yyy) {
     ...
     * @return s0
     */
    private static double multiply3(double a0, double a1, double a2, double b, MDD3 s12) {
     ...
     * @return s0
     */
    private static double add3(double a0, double a1, double a2, double b, MDD3 s12) {
     ...
     * @return s0
     */
    private static double add3(double a0, double a1, double a2, double b0, double b1, double b2, MDD3 s12) {
