...
class IndexSupportTest {
    ...
    @ParameterizedTest
    @MethodSource
    void testCheckIndex(int left, int right, int index) {
        Assertions.assertThrows(IndexOutOfBoundsException.class,
            () -> IndexSupport.checkIndex(left, right, index));
        final int[] k1 = new int[] {index};
        final int[] k2 = new int[] {left, right, index};
        final int[] k3 = new int[] {index, left, right};
        Assertions.assertThrows(IndexOutOfBoundsException.class,
            () -> IndexSupport.checkIndices(left, right, k1));
        Assertions.assertThrows(IndexOutOfBoundsException.class,
            () -> IndexSupport.checkIndices(left, right, k2));
        Assertions.assertThrows(IndexOutOfBoundsException.class,
            () -> IndexSupport.checkIndices(left, right, k3));
    }
    ...
    static Stream<Arguments> testCheckIndex() {
        ...
        builder.add(Arguments.of(0, 10, 11)); // upper bound is inclusive
        builder.add(Arguments.of(5, 10, 0));
        builder.add(Arguments.of(5, Integer.MAX_VALUE - 1, Integer.MAX_VALUE));
    ...
    void testCheckFromToIndex(int from, int to, int length) {
        Assertions.assertThrows(IndexOutOfBoundsException.class,
            () -> IndexSupport.checkFromToIndex(from, to, length));
    }
