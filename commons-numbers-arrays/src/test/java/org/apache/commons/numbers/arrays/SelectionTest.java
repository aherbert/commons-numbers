...
class SelectionTest {
    ...
    void testDoubleSelect(double[] values, int[] indices) {
        assertPartition(values, indices, (a, k, n) -> Selection.select(a, Arrays.copyOf(k, n)), false);
    }

    @ParameterizedTest
    @MethodSource(value = {"testDoublePartition", "testDoublePartitionBigData"})
    void testDoubleSelectRange(double[] values, int[] indices) {
        assertPartition(values, indices, (a, k, n) -> Selection.select(0, a.length, a, Arrays.copyOf(k, n)), false);
    }
