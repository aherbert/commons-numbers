 ...
 */
public final class LinearCombinationExact {
    /*
     * Caveat:
     *
    ...
    /** Private constructor. */
    private LinearCombinationExact() {
        // intentionally empty.
    }
    ...
    public static double value(double[] a,
        ...
        // q is the running scalar product
        double q = a[0] * b[0];
        e[0] = productLow(a[0], b[0], q);
        e[1] = q;

        // Remaining split products added to the current sum and round-off stored
        for (int i = 1; i < len; i++) {
            // Create the expansion f.
            final double f1 = a[i] * b[i];
            final double f0 = productLow(a[i], b[i], f1);
            q += f1;

            // Add the expansion f to e using expansion sum.
            // This uses two grow expansion operations.
            // The second can start at position 1.
            // The expansion sum algorithm for addition of expansion e and f of length
            // m and n requires mn two-sum operations. In this case n is always 2 so this
            // is 2m operations for a complexity of order(2m). The overall complexity
            // is order(n(n-1)/2) with n the length of the scalar product.
            // The number of operations may grow to be prohibitively slow but can be
            // reduced by ignoring zero parts during the expansion sum to limit the size.
            size = growExpansion(e, size, 0, f0);
            size = growExpansion(e, size, 1, f1);
        }

        // Sum the expansion
        return getSum(q, sum(e, size));
    }
