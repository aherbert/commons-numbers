...
final class BoostGamma {
    ...
    // be required to reduce error for extreme arguments. The implementation in
    // o.a.c.math3.FastMath has this functionality but it is not exposed as a public API.
    ...
    static final class Policy {
        /** Default policy. */
        private static final Policy DEFAULT = new Policy(EPSILON, 100000);
     ...
     * \[
     * \Gamma(x) = \frac{(z + g - 0.5)^{z - 0.5}}{e^{z + g - 0.5}} \mathrm{lanczos}(z)
     * \]
     * where \( g \) is the Lanczos constant.
     *
     * <h2>Warning</h2>
     *
     * <p>This is not a substitute for {@link LanczosApproximation}. The leading
     * factor of \( \sqrt{2\pi} \) is included in the sum.
     */
    ...
    static double tgamma(double z) {
        ...
        // Large non-integer z
        // Boost C++ tgamma implementation using absolute z
        double result = Lanczos.lanczosSum(absZ);
        final double zgh = absZ + Lanczos.g() - 0.5;
        final double lzgh = Math.log(zgh);
        if (absZ * lzgh > LOG_MAX_VALUE) {
            // we're going to overflow unless this is done with care:
            // Check for overflow has been removed:
            // if (lzgh * absZ / 2 > LOG_MAX_VALUE) ... overflow
            final double hp = Math.pow(zgh, (absZ / 2) - 0.25);
            result *= hp / Math.exp(zgh);
            // Check for overflow has been removed:
            // if (Double.MAX_VALUE / hp < result) ... overflow
            result *= hp;
        } else {
            result *= Math.pow(zgh, absZ - 0.5) / Math.exp(zgh);
        }
    ...
    static double lgamma(double z, int[] sign) {
        ...
        if (z <= -ROOT_EPSILON) {
            ...
            result = Sum.of(LOG_PI).add(-lgamma(z)).add(-Math.log(t)).getAsDouble();
        ...
        //} else if (z > 3 && z < 100) {
            ...
            // regular evaluation:
            final double zgh = z + Lanczos.g() - 0.5f;
     ...
     * Adapted from {@code boost::math::detail::gamma_p_derivative_imp}
     *
     * @param a Argument a
     * @param x Argument x
     * @return p derivative
     ...
     * @param invert true to compute the upper value (default is lower)
    ...
    private static double gammaIncompleteImp(double a, double x,
        ...
        boolean isInt;
        boolean isHalfInt;
        final boolean isSmallA = (a < 30) && (a <= x + 1) && (x < LOG_MAX_VALUE);
        if (isSmallA) {
     ...
     */
    private static double upperGammaFraction(double a, double z, Policy pol) {
        final double eps = pol.getEps();
        final int maxIterations = pol.getMaxIterations();

        // Commons Number 1.0 RegularizedGamma.Q:
        //
        // final ContinuedFraction cf1 = new ContinuedFraction() {
        //     /** {@inheritDoc} */
        //     @Override
        ...
        // };
        //
        // return 1 / cf1.evaluate(z, eps, maxIterations);

        // Custom continued fraction based on the fraction terms used in the Boost implementation.
        //
        // TODO: Determine if this can be replaced by the generic ContinuedFraction class.
        // Both are using the same Lentz algorithm with some differences:
        //
        // The tiny value is much smaller: 1e-50 vs 3.5601181736115222e-307.
        //
        // The evaluation omits the n=0 term for b0 + fraction. That is added after evaluation.
        // In this usage it is be z - a + 1. In addition the term a1 is reserved (in
        // evaluate(...) this is named a0) and divided by the fraction at the end.
        // The same effect with ContinueFraction would be to start at b1 for n=0 and
        // then compute the final result using the true b0 and a1 that would not be used
        // during fraction evaluation, i.e. b0 + (a1 / cf.evaluate()).
        //
        // The termination is based on 'continue if (Math.abs(delta - 1) > terminator'
        // rather than 'stop if Math.abs(deltaN - 1) < epsilon'. This is likely a negligible
        // difference.
        //
        // There is no computation of the next value (f) and checking it is not finite.
        // The next value is computed without checks. The ContinueFraction class is
        // more robust. However in this case if the value is infinite then the result is:
        // a0 / inf = 0. This is a usable result.
        //
        // The Boost version never uses a max iterations and eps is set to the machine epsilon.
        // The fraction should be evaluated when convergence is expected. Alternative evaluation
        // of the gamma function is used when the fraction convergence is slow.
        // The Boost code never checks the number of iterations.

        final Supplier<double[]> cf = new Supplier<double[]>() {
            /** Factor. */
            ...
            public double[] get() {
                ++k;
                x += 2;
                return new double[] {k * (a - k), x};
            }
        };

        return 1 / (z - a + 1 + BoostTools.continuedFractionA(cf, eps, maxIterations));
    }

    /**
     * Upper gamma fraction for integer a.
     * Called when a < 30 and x < LOG_MAX_VALUE.
     *
    ...
    private static double finiteGammaQ(double a, double x) {
        ...
        // Update:
        // Assume x < log max value and no underflow of e to zero.
    ...
    private static double finiteHalfGammaQ(double a, double x) {
        ...
        // Update:
        // Assume x < log max value:
        // erfc(sqrt(709)) = erfc(26.6) => erfc has a non-zero value
     ...
     */
    private static double lowerGammaSeries(double a, double z, double initValue, Policy pol) {
        final double eps = pol.getEps();
        final int maxIterations = pol.getMaxIterations();

        //return BoostTools.sumSeries(new LowerIncompleteGammaSeries(a, z), eps, maxIterations, initValue);
        return BoostTools.kahanSumSeries(new LowerIncompleteGammaSeries(a, z), eps, maxIterations, initValue);
    }

    /**
     * Lower gamma series representation.
     */
    private static class LowerIncompleteGammaSeries implements DoubleSupplier {
        /** Argument a. */
        private double a;
        /** Argument z. */
         ...
         * @param z Argument z
         */
        LowerIncompleteGammaSeries(double a, double z) {
            this.a = a;
            this.z = z;
            result = 1;
        }
     ...
     * @param x Argument x
     * @param pol the pol
     * @param pgam set to value of gamma(a)
    ...
    private static double tgammaSmallUpperPart(double a, double x, Policy pol, double[] pgam, boolean invert) {
        ...
        final int maxIter = pol.getMaxIterations(); // - 10;
        p += 1;
        final double initValue = invert ? pgam[0] : 0;
        //result = -p * BoostTools.sumSeries(
        //        new SmallGamma2Series(a, x), pol.getEps(), maxIter, (initValue - result) / p);
        result = -p * BoostTools.kahanSumSeries(
                new SmallGamma2Series(a, x), pol.getEps(), maxIter, (initValue - result) / p);
        if (invert) {
            result = -result;
        }
     ...
     */
    private static class SmallGamma2Series implements DoubleSupplier {
        /** The result. */
        private double result;
        /** Argument x (this is negated). */
         ...
         * @param x Argument x
         */
        SmallGamma2Series(double a, double x) {
            this.x = result = -x;
            apn = a + 1;
            n = 1;
        }
     ...
     */
    private static double regularisedGammaPrefix(double a, double z) {
        if (z >= Double.MAX_VALUE) {
            return 0;
        }
        ...
        if (a < 1) {
            ...
            // -z > log(min_value)
            if (-z <= LOG_MIN_VALUE) {
                // Oh dear, have to use Math.logs, should be free of cancellation errors though:
        ...
        final double agh = a + Lanczos.g() - 0.5;
        double prefix;
        final double d = ((z - a) - Lanczos.g() + 0.5) / agh;

        if ((Math.abs(d * d * a) <= 100) && (a > 150)) {
            // special case for large a and a ~ z.
            prefix = a * SpecialMath.log1pmx(d) + z * (0.5 - Lanczos.g()) / agh;
     ...
     * </pre>
     *
     * <p>This is a port of the function accurate for 53-bit mantissa's
     ...
     * @return the double
     */
    private static double igammaTemmeLarge(double a, double x) {
     ...
     */
    private static double incompleteTgammaLargeX(double a, double x, Policy pol) {
        final double eps = pol.getEps();
        final int maxIterations = pol.getMaxIterations();

        return BoostTools.sumSeries(new IncompleteTGammaLargeXSeries(a, x), eps, maxIterations);
    }

    /**
     * Asymptotic approximation for large argument, see: https://dlmf.nist.gov/8.11#E2.
     */
    private static class IncompleteTGammaLargeXSeries implements DoubleSupplier {
        /** Argument {@code a - n}. Named a_poch in the Boost code. */
        private double amn;
        /** Argument z. */
        private final double z;
        /** The result. */
         ...
         * @param x Argument x
         */
        IncompleteTGammaLargeXSeries(double a, double x) {
            this.amn = a - 1;
            this.z = x;
            term = 1;
        }
     ...
     *
     * @param array Array
     * @return true if a non-zero length
     */
